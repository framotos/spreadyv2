...
def decode_jwt(token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # FIXME: Enable signature verification as per Task 1.1
        payload = jwt.decode(
            token,
        ...
        if "iss" not in payload or not (settings.SUPABASE_URL and payload["iss"].startswith(f"{settings.SUPABASE_URL}/auth/v1")):
